...
if ! git rev-parse -q --verify origin/main >/dev/null 2>&1; then
  trava "origin/main não existe neste clone — adicione o remoto do repositório principal: git remote add upstream https://github.com/sysadmin-stack/DeskcommCRM.git && git fetch upstream && git branch --track main-upstream upstream/main"
  exit 0
fi
...
if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
  checks="$(gh api repos/sysadmin-stack/DeskcommCRM/branches/main/protection --jq '.required_status_checks.contexts|join(", ")' 2>/dev/null || true)"
